    ...
    return new Elysia({
        ...
        .decorate('unsignCookie', ((value: string) => {
            ...
            let unsigned: false | string = isStringKey
                ...
                if (isStringKey) {
                    unsigned = unsign(value, secret);
                } else if (Array.isArray(secretKey)) { // ตรวจสอบว่ามันเป็นอาเรย์
                    for (let i = 0; i < secretKey.length; i++) { // ใช้ secretKey แทน secret
                        const temp = unsign(value, secretKey[i]); // ใช้ secretKey[i] แทน secret[i]
